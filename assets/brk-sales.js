...
(function (global) {
  ...
  var CSS =
    ...
    ".brks .sp{flex:1}" +
    ".brks .slk{display:flex;flex-wrap:wrap;align-items:center;gap:8px 14px;margin-top:14px;padding-top:14px;border-top:1px solid #F1E6DE;font-size:12.5px;color:#6E5E55}" +
  ...
  function mountUploader(box, opt) {
    ...
    box.innerHTML =
      ...
      (opt.slackPayload ?
      "<div class='slk'><span>슬랙 알림 <span id='brksSlkState'>확인 중…</span></span>" +
        "<button type='button' class='lnk' id='brksSlkNow'>지금 매출을 슬랙에 보내기</button>" +
        "<button type='button' class='lnk' id='brksSlkSet'>슬랙 연결 설정</button></div>" +
    ...
    function slackPref() { try { return localStorage.getItem("brks_slack") !== "0"; } catch (e) { return true; } }
    function paintSlack() {
    ...
    function sendSlack(key, M) {
      ...
      var pl = opt.slackPayload(M || model, by);
      if (!pl) return Promise.reject(new Error("보낼 매출이 없습니다"));
    ...
    function preview() {
      ...
      pv.innerHTML =
        ...
        "<div class='act'>" +
          ...
          (opt.slackPayload && model && model.slack ? "<label class='chk'><input type='checkbox' id='brksSlkOn'" + (slackPref() ? " checked" : "") + "> 슬랙에도 올리기</label>" : "") +
      ...
      $("brksGo").onclick = go;
      if ($("brksSlkOn")) $("brksSlkOn").onchange = function () { try { localStorage.setItem("brks_slack", this.checked ? "1" : "0"); } catch (e) {} };
    }
