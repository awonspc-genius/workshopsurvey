...
(function (global) {
  ...
  function build(base, sheet) {
    var M = { days: {}, mon: {}, base: base || null, sheetOk: !!(sheet && sheet.ok),
              ...
              sheetRows: 0, lastUpload: null, lastBy: "", slack: !!(sheet && sheet.slack) };
  ...
  var RX = {
    ...
    bad:   /비중|구성|%|률|율|比|대비|증감|객단가|단가|수량|목표|누계|누적|평균|순번|^no|매장수|점포수|순위|신장|차이|성장|계획|예상|전망|기여|index|조직|코드|번호|점포/i,
  ...
  function parseSheet(rows, sheetName, fileName) {
    ...
    /* 묶음 제목(예: 조회기간 / 전년 동 요일)은 오른쪽 빈칸으로 이어 준다 (마지막 제목 줄은 제외) */
    var carry = {};
    for (var hr0 = hTop; hr0 < best.first - 1; hr0++) {
      var cur = null;
      for (var c0 = 0; c0 < maxC; c0++) {
        var t0 = txt(cell(hr0, c0));
        if (t0) cur = t0;
        else if (cur && c0 > best.c) (carry[hr0] = carry[hr0] || {})[c0] = cur;
      }
    }
    for (var cc = 0; cc < maxC; cc++) {
      var parts = [];
      for (var hr = hTop; hr < best.first; hr++) {
        var t = txt(cell(hr, cc)) || (carry[hr] && carry[hr][cc]) || "";
    ...
    /* 매출 칸이 여럿이면 '실매출'이 들어간 칸, 없으면 값이 가장 큰 칸을 쓴다 */
    (function () {
      var cs = colsOf("s"); if (cs.length < 2) return;
      var tot = function (i) { var t = 0; for (var r = best.first; r < rows.length; r++) { var v = toNum(cell(r, i)); if (v) t += Math.abs(v); } return t; };
      var pick = cs.filter(function (i) { return /실매출/.test(labels[i]); })[0];
      if (pick === undefined) pick = cs.slice().sort(function (a, b) { return tot(b) - tot(a); })[0];
      cs.forEach(function (i) { if (i !== pick) colK[i] = null; });
    })();
    /* 전년 칸이 여럿이면(전년 동일 / 전년 동요일) 동요일 쪽을 쓴다 */
    ["pw", "pwc"].forEach(function (kk) {
      var cs = colsOf(kk); if (cs.length < 2) return;
      var pick = cs.filter(function (i) { return /동요일/.test(labels[i].replace(/\s/g, "")); })[0];
  ...
  function parseFiles(files) {
    ...
    return ensureXLSX().then(function (X) {
      return Promise.all(files.map(function (f) {
        ...
        var R = { days: {}, mon: {}, sheets: [], skipped: [], notes: [], unitGuess: 1, unitFound: null };
        var all = [].concat.apply([], lists).filter(function (sh) {
          return Object.keys(sh.days).length || Object.keys(sh.mon).length;
        });
        /* 어떤 탭을 쓸지 고르기
           ① 이름에 RAW·BACK 이 들어간 탭(제품별 원자료)은 빼고
           ② 날짜가 세로로 내려가는 일별 표가 있으면 가로 요약표는 빼고
           ③ 가장 최근 날짜가 든 탭과 겹치지 않는 옛날 탭(작년 일매출 등)은 뺀다 */
        var keep = all.filter(function (sh) { return !/RAW|BACK/i.test(sh.sheet); });
        if (!keep.length) keep = all;
        if (keep.some(function (sh) { return sh.mode === "일별"; }))
          keep = keep.filter(function (sh) { return sh.mode !== "일별(가로)"; });
        var maxOf = function (sh) { return Object.keys(sh.days).concat(Object.keys(sh.mon)).sort().pop() || ""; };
        var minOf = function (sh) { return Object.keys(sh.days).concat(Object.keys(sh.mon)).sort()[0] || ""; };
        var latest = keep.map(maxOf).sort().pop() || "";
        var latestMonth = latest.slice(0, 7);
        var newest = keep.filter(function (sh) { return maxOf(sh) === latest; })[0];
        var floor = newest ? minOf(newest).slice(0, 7) : "";
        keep = keep.filter(function (sh) { return maxOf(sh).slice(0, 7) >= floor; });
        all.forEach(function (sh) { if (keep.indexOf(sh) < 0) R.skipped.push(sh.sheet); });
        keep.forEach(function (sh) {
  ...
  /* 스크립트에 한 건 보내고 답 받기 (답을 못 읽으면 blind:true) */
  function postOne(obj) {
    var hd = { "Content-Type": "text/plain;charset=utf-8" }, body = JSON.stringify(obj);
    return fetch(DATA_URL, { method: "POST", headers: hd, body: body })
      .then(function (r) { return r.json(); })
      .then(function (j) {
        if (!j || !j.ok) { var e = new Error((j && j.error) || "실패했습니다"); e.hard = true; throw e; }
        return j;
      })
      .catch(function (e) {
        if (e && e.hard) throw e;
        return fetch(DATA_URL, { method: "POST", mode: "no-cors", headers: hd, body: body })
          .then(function () { return { ok: true, blind: true }; });
      });
  }
  function slackSetup(conf, key) { return postOne({ type: "slackSetup", key: key, conf: conf }); }
  function slackSend(payload, key) { return postOne({ type: "slack", key: key, payload: payload }); }
  ...
  var CSS =
    ...
    ".brks .mapd{margin-top:10px;font-size:12px;color:#6E5E55}.brks .mapd summary{cursor:pointer}" +
    ...
    ".brks .sp{flex:1}" +
    ".brks .slkd{margin-top:14px;padding-top:12px;border-top:1px solid #F1E6DE}" +
    ".brks .slkd summary{cursor:pointer;font-size:13px;color:#6E5E55;list-style:none;display:inline-flex;align-items:center;gap:6px}" +
    ".brks .slkd summary::-webkit-details-marker{display:none}" +
    ".brks .slkd summary::before{content:'▸';font-size:11px;transition:transform .15s}" +
    ".brks .slkd[open] summary::before{transform:rotate(90deg)}" +
    ".brks .slkd summary:focus-visible{outline:2px solid #000;outline-offset:3px;border-radius:4px}" +
    ".brks .slk{display:flex;flex-wrap:wrap;align-items:center;gap:8px 14px;margin-top:10px;font-size:12.5px;color:#6E5E55}" +
    ".brks .slk b{color:#000}.brks .slk .on{color:#0F6E56;font-weight:800}.brks .slk .off{color:#9A5B00;font-weight:800}" +
    ".brks .slkf{margin-top:12px;padding:14px;background:#FBF6F2;border-radius:12px;display:flex;flex-direction:column;gap:10px}" +
    ".brks .slkf .r{display:flex;flex-wrap:wrap;gap:8px;align-items:center}" +
    ".brks .slkf input[type=text]{flex:1;min-width:260px;width:auto}" +
    ".brks .slkf p{margin:0;font-size:12px;color:#6E5E55;line-height:1.7}" +
    ".brks label.chk{display:flex;align-items:center;gap:6px;font-size:13px;color:#000;cursor:pointer}" +
  ...
  function mountUploader(box, opt) {
    ...
    box.innerHTML =
      ...
      "<div class='slk'>" +
        ...
        "<button type='button' class='lnk' id='brksSlkSet'>슬랙 연결 설정</button>" +
        "<span>엑셀을 반영하면 슬랙에도 자동으로 올라갑니다.</span></div>" +
      "<div class='slkf' id='brksSlkF' hidden>" +
        "<div class='r' id='brksSlkUrlRow'><input type='text' id='brksSlkTok' placeholder='봇 토큰 xoxb-…' aria-label='슬랙 봇 토큰' autocomplete='off'>" +
          "<input type='text' id='brksSlkCh' placeholder='채널 ID  C0…' aria-label='슬랙 채널 ID' style='flex:0 1 170px;min-width:150px' autocomplete='off'>" +
          "<button type='button' class='ghost' id='brksSlkSave'>연결하고 시험 글 보내기</button>" +
          "<button type='button' class='lnk' id='brksSlkOff'>연결 끊기</button></div>" +
        "<div class='r'><input type='password' id='brksSlkKey' placeholder='비밀번호' aria-label='비밀번호' autocomplete='off'>" +
          "<button type='button' class='go' id='brksSlkGo' hidden>슬랙에 보내기</button></div>" +
        "<p id='brksSlkHelp'>슬랙 앱의 Bot User OAuth Token(xoxb-)과 올릴 채널의 ID를 넣으세요. 앱을 채널에 먼저 초대해 두어야 합니다. 토큰은 구글 스크립트 안에만 저장되고 사이트 파일에는 남지 않습니다.</p>" +
      "</div></details>" : "");
    ...
    function paintSlack() {
      var st = $("brksSlkState"); if (!st || !model) return;
      st.innerHTML = model.slack ? "<span class='on'>연결됨</span>" : "<span class='off'>연결 안 됨</span>";
      if ($("brksSlkNow")) $("brksSlkNow").hidden = !model.slack;
    }
    function sendSlack(key, M) {
      var by = ""; try { by = localStorage.getItem("brks_by") || ""; } catch (e) {}
      return Promise.resolve(opt.slackPayload(M || model, by)).then(function (pl) {
        if (!pl) throw new Error("보낼 매출이 없습니다");
        return slackSend(pl, key);
      });
    }
    ...
    function preview() {
      ...
      pv.innerHTML =
        ...
        "<ul class='map'><li><b>읽은 탭</b>: " + esc(R.sheets.map(function (s) { return s.sheet; }).join(", ")) +
          (R.skipped.length ? " · <b>뺀 탭</b>: " + esc(R.skipped.join(", ")) : "") + "</li>" + R.sheets.map(function (s) {
          return "<li><b>" + esc(s.sheet) + "</b> (" + s.mode + ", " + (s.days ? s.days + "일" : s.months + "달") + "): " + esc(s.map.join(" · ") || "칸 이름 없음") + "</li>";
        }).join("") + "</ul></details>" +
        (warn.length ? "<div class='warn'>" + warn.map(esc).join("<br>") + "</div>" : "") +
        "<div class='act'>" +
          /* 금액 단위 칸은 단위를 엑셀에서 못 찾고 '원'이 아닌 것으로 짐작했을 때만 보여 준다 */
          (!R.unitFound && R.unitGuess !== 1 ?
          "<label class='u'>금액 단위 확인 <select id='brksUnit'>" + units.map(function (u) {
            return "<option value='" + u[0] + "'" + (u[0] === unit ? " selected" : "") + ">" + u[1] + (u[0] === R.unitGuess ? " (짐작)" : "") + "</option>";
          }).join("") + "</select></label>" : "<input type='hidden' id='brksUnit' value='" + unit + "'>") +
          "<span class='sp'></span>" +
          ...
          (opt.slackPayload && model && model.slack ? "<label class='chk'><input type='checkbox' id='brksSlkOn' checked> 슬랙에도 알림 보내기</label>" : "") +
    ...
    function go() {
      ...
      upload(recs, key, by).then(function (res) {
        return load({ fresh: true }).then(function (M) {
          ...
          if (wantSlack) {
            /* 페이지 숫자가 새로 칠해진 뒤 요약을 만든다 */
            setTimeout(function () {
              sendSlack(key, M).then(function (j) { say(done + " 슬랙에도 올렸습니다." + (j && j.note ? " " + esc(j.note) : ""), "ok"); })
                .catch(function (e) { say(done + " 다만 슬랙에는 못 올렸습니다: " + esc(e.message || e), "err"); });
            }, 300);
          }
        });
      }).catch(function (e) {
    ...
    if (opt.slackPayload && opt.admin) {   /* 슬랙 연결 설정은 관리자 주소(/plan/admin)에서만 */
      ...
      var openF = function (m) {
        ...
        $("brksSlkUrlRow").hidden = m !== "set";
        $("brksSlkGo").hidden = m !== "send";
        $("brksSlkHelp").hidden = m !== "set";
        (m === "set" ? $("brksSlkTok") : $("brksSlkKey")).focus();
      };
      $("brksSlkSet").onclick = function () { if (!f.hidden && mode === "set") f.hidden = true; else openF("set"); };
      $("brksSlkNow").onclick = function () { if (!f.hidden && mode === "send") f.hidden = true; else openF("send"); };
      var needKey = function () {
        var k = $("brksSlkKey").value.trim();
        if (!k) { say("비밀번호를 넣어 주세요.", "err"); $("brksSlkKey").focus(); }
        return k;
      };
      var setUrl = function (conf) {
        var k = needKey(); if (!k) return;
        var on = !!conf.token;
        say(on ? "슬랙에 연결하는 중…" : "연결을 끊는 중…");
        slackSetup(conf, k).then(function (j) {
          model.slack = on; paintSlack(); f.hidden = true; $("brksSlkTok").value = ""; $("brksSlkCh").value = "";
          say(on ? (j.blind ? "연결을 보냈습니다. 슬랙 채널에 시험 글이 올라왔는지 확인해 주세요." : "연결했습니다. 슬랙 채널에 시험 글이 올라갔습니다.") : "슬랙 연결을 끊었습니다.", "ok");
          load({ fresh: true }).then(function (M) { model = M; paintSlack(); });
        }).catch(function (e) { say(esc(e.message || e), "err"); });
      };
      $("brksSlkSave").onclick = function () {
        var t = $("brksSlkTok").value.trim(), c = $("brksSlkCh").value.trim();
        if (!t || !c) { say("봇 토큰과 채널 ID를 둘 다 넣어 주세요.", "err"); return; }
        setUrl({ token: t, channel: c });
      };
      $("brksSlkOff").onclick = function () { if (confirm("슬랙 알림 연결을 끊을까요?")) setUrl({ token: "", channel: "" }); };
      $("brksSlkGo").onclick = function () {
        var k = needKey(); if (!k) return;
        say("슬랙에 보내는 중…");
        sendSlack(k).then(function (j) { f.hidden = true; say(j.blind ? "슬랙에 보냈습니다. 채널을 확인해 주세요." : ("슬랙에 올렸습니다." + (j.note ? " " + esc(j.note) : "")), "ok"); })
          .catch(function (e) { say("슬랙에 못 올렸습니다: " + esc(e.message || e), "err"); });
      };
      $("brksSlkKey").onkeydown = function (e) { if (e.key === "Enter") (mode === "set" ? $("brksSlkSave") : $("brksSlkGo")).click(); };
    }
  }

  global.BRKSales = {
    load: load, parseFiles: parseFiles, toRecords: toRecords, upload: upload,
    mountUploader: mountUploader, template: template, slackSend: slackSend, slackSetup: slackSetup,
